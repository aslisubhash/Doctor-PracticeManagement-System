...
class prescription_model extends CI_Model 
{
	...
	function get_prescription_by_doctor()
	{
			$admin = $this->session->userdata('admin');
				if($admin['user_role']==1){
					$this->db->where('P.doctor_id',$admin['id']);	
				   }else{
					$this->db->where('P.doctor_id',$admin['doctor_id']);	
				   }
	...
	function get_payment_by_doctor()
	{
		$admin = $this->session->userdata('admin');
		if($admin['user_role']==1){
					$this->db->where('F.doctor_id',$admin['id']);	
				   }else{
					$this->db->where('F.doctor_id',$admin['doctor_id']);	
				   }
		
		$this->db->order_by('F.invoice','DESC');
		$this->db->select('F.*,PM.name mode,U.name patient,D.name doctor');
